...
async def async_setup_entry(
    ...
    """Set up the Shelly BLU TRV external temperature number entity."""
    coordinator: ShellyBluTrvCoordinator = hass.data[DOMAIN][entry.entry_id]
    async_add_entities([ShellyBluTrvExternalTemp(coordinator)])
...
class ShellyBluTrvExternalTemp(ShellyBluTrvEntity, NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        try:
            ...
            await self.coordinator.client.async_set_external_temperature(value)
        finally:
            try:
